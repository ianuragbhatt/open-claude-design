...
export function PreviewPane({
  ...
  // Measure container width for zoom-to-fit desktop preview
  React.useEffect(() => {
  ...
  // Keyboard shortcuts for viewports and modes
  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "1") setViewport("responsive");
      else if (e.key === "2") setViewport("desktop");
      else if (e.key === "3") setViewport("tablet");
      else if (e.key === "4") setViewport("mobile");
      else if (e.key === "i" || e.key === "I")
        setCanvasMode((prev) => (prev === "interact" ? "inspect" : "interact"));
  ...
  const displayCode = isLoading && streamingCode ? streamingCode : htmlToDisplay;

  // Compute scale for Desktop viewport
  const fitScale = Math.min(1, Math.max(0.35, (containerWidth - 48) / 1440));
  const activeDesktopScale =
    desktopZoom === "fit"
      ? fitScale
      : desktopZoom === "100"
      ? 1
  ...
  return (
    <div className="flex-1 h-full flex flex-col bg-background overflow-hidden transition-colors">
      {/* Top Ribbon - Khayal Command Ribbon */}
      <div className="h-12 px-4 border-b border-border bg-surface flex items-center justify-between shrink-0 select-none z-20 text-xs text-foreground transition-colors">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {/* Zoom Controls for Desktop Mode */}
          {viewport === "desktop" && (
            <div className="hidden lg:flex items-center p-0.5 bg-surface-subtle/80 rounded-lg border border-border/60 text-[10px]">
              {(["fit", "100", "75", "50"] as const).map((z) => (
                <button
                  ...
                  title={z === "fit" ? "Scale to fit window" : `Zoom ${z}%`}
                >
                  {z === "fit" ? `Fit (${Math.round(fitScale * 100)}%)` : `${z}%`}
                </button>
              ))}
            </div>
          )}
          ...
          <div className="flex items-center p-0.5 bg-surface-subtle/80 rounded-xl border border-border/60">
            <button
              ...
              title="Test interactive prototype buttons & links (Key: I)"
            >
            ...
            <button
              ...
              title="Click any section on the canvas to request targeted revisions (Key: I)"
            >
                ...
                <div className="absolute right-0 top-full mt-1.5 w-60 bg-surface border border-border rounded-2xl shadow-2xl p-1.5 z-50 text-xs animate-in fade-in zoom-in-95 duration-100">
                  ...
                  {onExportProjectJson && (
                    ...
                    >
                      ...
                      <span>Export Project (.khayal.json)</span>
                    </button>
                  )}
      ...
      <div
        ref={containerRef}
        className="flex-1 overflow-auto flex items-center justify-center p-6 bg-background relative"
        style={{
      ...
      >
        {activeTab === "preview" ? (
          viewport === "desktop" ? (
            <div
              style={{
                width: `${1440 * activeDesktopScale}px`,
                height: `${900 * activeDesktopScale}px`,
              }}
            ...
            >
              <div
                style={{
                  width: "1440px",
                  height: "900px",
                  transform: `scale(${activeDesktopScale})`,
